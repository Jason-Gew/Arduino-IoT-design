...
boolean PubSubClient::write(uint8_t header, uint8_t buf [200], uint16_t length) {
   uint8_t lenBuf[4];
   uint8_t llen = 0;
   uint8_t digit;
   uint8_t pos = 0;
   ...
   buf[4-llen] = header;      //buffer 0..4 are part of header, header, then length
   for (int i=0;i<llen;i++) {
      buf[5-llen+i] = lenBuf[i];  
   }
   
   //modified by Scott Carlson
   //rc = _client->write(buf+(4-llen),length+1+llen);
   int i;
   for(i=0;i<length+1+llen;i++){
	 rc+=_client->print((char)(buf[4-llen+i]));
	 //Serial.print((char)(buf[4-llen+i]));
	 delay(10);
	}
...
uint16_t PubSubClient::writeString(char* string, uint8_t* buf, uint16_t pos) {
   char* idp = string;   //idp = "arduino_mega
   uint16_t i = 0;       //i=0
   pos += 2;             //pos=7
   while (*idp) {        //foreach in idp
      buf[pos++] = *idp++; //pos=8,9,10,... buf[  
      i++;                 //i=1,2,3
   }
   buf[pos-i-2] = (i >> 8);   //i/256, position 5    
   buf[pos-i-1] = (i & 0xFF); //i, position 6  
   return pos;
}
